...
etoh_m <- AHMdata %>% 
  ...
  mutate(highalc = ifelse(is.na(highalc), 0, highalc)) %>% #Leigh treats missing values as no alcohol intake
  rename(
    age = mv012
  ) %>% 
  mutate(age2 = age^2, age3 = age^3) %>% 
  select(age, age2, age3, province, highalc)
```
...
```
### Model alcohol ~ age + province (cubic function)
...
for (n in 1:9) {
  ...
  etoh_prev_m <- bind_rows(etoh_prev_m, temp)
}
...
for (n in 1:9) {
  ...
  etoh_prev_w <- bind_rows(etoh_prev_w, temp)
}
